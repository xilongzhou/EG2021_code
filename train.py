...
if __name__ == '__main__':
	...
	if opt.logs:
		...
		# writer = SummaryWriter('MyLogs/VGG_overfitting_smalldata')
	else:
		print('not logging')
	...
	for epoch in range(start_epoch, opt.niter + opt.niter_decay + 1):
		...
		for i, data in enumerate(dataset, start=epoch_iter):
			...
			############## Forward Pass ######################
			losses, generated, rendered, inputimage, light, real, real_l1loss = model(Variable(data['label']), Variable(data['inst']), 
				Variable(data['image']), Variable(data['feat']), Variable(real_data_pair), infer=save_fake)
			...
			if opt.real_train:
				...
				if opt.fp16:                                
					with amp.scale_loss(loss_D_render_real, optimizer_D_render_real) as scaled_loss: scaled_loss.backward()                
				else:
